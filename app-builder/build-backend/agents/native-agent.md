...
* **Prefix (optional)**: Enter a [prefix](native-agent.md#using-a-prefix-for-fleet-deployment) if you plan to deploy the same file to multiple machines (e.g., `milling`). Lowercase letters and digits only. Each instance then generates its own unique ID (e.g., `milling-native-agent-abc12`).
...
* **With a prefix**: The Agent generates a new, unique ID on its first launch in a specific directory. Deploy the exact same installer on multiple machines and each one connects as a separate entry in your Function Explorer (e.g., `milling-native-agent-abc12`, `milling-native-agent-xyz78`).
