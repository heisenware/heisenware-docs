...
When enabling _Center on markers_, the map calculates the geographic midpoint to position the view. This center calculation can push some markers outside the view boundary if the _Default zoom_ scale is too restrictive. Adjust the _Default zoom_ setting to fit all coordinate points into the visible display area.
{% endhint %}
