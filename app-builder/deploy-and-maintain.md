...
* **What a `.hwt` contains**: your App's configuration plus every file from the `uploads` and `project` folders it references (images, CSVs, documents), so the App looks and works the same on another server. Files that already exist on the target keep their local copy. Runtime files and backups are not included. **Secrets never travel**: secret variables and secret inputs arrive empty, and the import summary lists the ones you have to enter, together with any agents the App uses that the target server does not have yet.
