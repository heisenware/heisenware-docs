...
The [Heisenware architecture](https://app.gitbook.com/o/cRJpmFHZiztu0UZzh1Up/s/E5Ketpww1s7TauSAJrJ8/) consists of the central platform and optional Agents. Each account (tenant) runs in isolation and supports two deployment modes:
...
* **Visual with full code access:** You build logic visually in the [Backend Builder](https://docs.heisenware.com/app-builder/build-backend) and keep direct access to developer tools like JavaScript expressions for data transformation and YAML for configuration.
* **Extensible:** When the built-in [Functions](https://docs.heisenware.com/app-builder/build-backend/functions) are not enough, you wrap your own code (Node.js, Python, C++) into extensions that become native Functions.
...
* **The bridge**: Through [Native Agents](app-builder/build-backend/function-explorer/agents/native-agent.md) and [Docker Agents](https://docs.heisenware.com/app-builder/build-backend/function-explorer/agents/docker-agent), we securely bridge local, private networks (OT/Shopfloor) to the cloud without VPNs.
* **Local execution**: You can push backend logic ([Connectors](https://docs.heisenware.com/app-builder/build-backend/function-explorer/connectors)) to run locally on the edge device. The platform treats these remote Functions the same as cloud Functions.
